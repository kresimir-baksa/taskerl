...
test_two_tasks_sorting(_Config) ->
    {ok, RequestBody} = load_from_inputs("two_tasks.json"),
    {ok, {{_, 200, _}, _, Body}} =
        httpc:request(post,
                      {"http://localhost:8080/tasks/json", [], "application/json", RequestBody},
                      [],
                      []),
    ...
    {ok, {{_, 200, _}, _, BodySh}} =
        httpc:request(post,
                      {"http://localhost:8080/tasks/shell", [], "application/json", RequestBody},
                      [],
                      []),
...
test_multiple_tasks_sorting(_Config) ->
...
test_bad_requests(_Config) ->
    {ok, {{_, 500, _}, _, _}} =
        httpc:request(post,
                      {"http://localhost:8080/tasks/json", [], "application/json", <<"{}">>},
                      [],
                      []),
    {ok, {{_, 404, _}, _, _}} =
        httpc:request(post,
                      {"http://localhost:8080/tasks", [], "application/json", <<"{}">>},
                      [],
                      []),
    ok.
